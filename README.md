...
[Настройка проекта](#Настройка-проекта) [en-README]()
## Описание
Участник дискорд сервера может при использовании команд этого бота:
 ...
 - [покажимне](#покажимне-[@UserMention]) :просмотреть список и количество карт одного типа, которые он уже получил
 - [профиль](#профиль-[@UserMention]) : просмотреть статистику пользователя сервера по выпавшим картам 
 ...
 - [выдайкарту](#выдайкарту-@UserMention-CardName) : выдать 1 карту указанному пользователю
 - [активируй](#активируй-Code) : активировать код для получения шанса получить катру еще раз
 - [создатькод](#создатькод-[-c]-[-u]-[-d]) : создать код, который могут использовать пользователи для получения возможности получить карту
 - [удалитькарту](#удалитькарту-CardName) : удалить карту из системы и у всех пользователей
 - [новаякарта](#новаякарта-CardName-ClassNumber-[ImageSourceLink]) : добавить в пул выпадения карт новую карту
 - [изменитькарту](#изменитькарту-CardName-editCardName-editClassNumber-[editImageSourceLink]) : изменить существующую карту в системе и у всех пользователей
 - [обновикрутки](#обновикрутки-[@UserMention]) : дать возможность всем или конкретному пользователю получить карту
### дайкарту
