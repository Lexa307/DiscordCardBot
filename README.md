...
## Новости
 **фича от 09.12.2021**
 - теперь можно установить серверное время в момент которого, таймер всех пользователей для получения карты(**daikarty**) обнуляется.
...
**RESET_LOCAL_TIME** - локальное время сервера во время которого происходит сброс таймера для **daikarty** у всех пользователей одновременно. Формат значения поля: **"чч:мм:сс"**, например **RESET_LOCAL_TIME** = "16:45:00".  Если **""** - то при использовании **daikarty** бот будет устанавливать время сброса каждому пользователю индивидуально.
